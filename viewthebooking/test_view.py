...
class TestViewBooking(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='test11',
                                             password='12345')
        self.client = Client()
        self.client.login(username='test11', password='12345')
        self.booking1 = BookingRequest.objects.create(
            user=self.user,
            choose_a_services='Waste',
            time_slot='7:00',
            date='2024-04-15'
        )
        self.booking2 = BookingRequest.objects.create(
            user=self.user,
            choose_a_services='Transfer',
            time_slot='9:00',
            date='2024-04-16'
        )
    ...
    def test_view_booking(self):
        ...
        self.assertTemplateUsed(response,
                                'viewthebooking/view_the_booking.html')
        expected_queryset = BookingRequest.objects.filter(
            user=self.user
        ).order_by('created_at')
